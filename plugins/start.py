...
FORCE_SUB_CHANNEL_IDS = {
    "channel": "GenAnimeOfc",
    "channel2": "FusionAnime",
    "channel3": "DS_AnimeX",
   # "channel3": "GenAnimeHindi",
  #  "channel4": "GenAnimeChat",
 #   "channel5": "Loner_Life_in_Another_World_01",
...
async def start_command(client: Client, message: Message):
    for sub_channel_id in FORCE_SUB_CHANNEL_IDS.values():
        try:
            ...
            if user.status == "Kɪᴄᴋᴇᴅ Oᴜᴛ":
                await message.reply_text("Yᴏᴜʀ Aʀᴇ Bᴀɴɴᴇᴅ")
                return
        except UserNotParticipant:
            await message.reply_text(
                text="❗ Yᴏᴜ Aʀᴇ Nᴏᴛ ᴀ Mᴇᴍʙᴇʀ Oꜰ Oᴜʀ Nᴇᴛᴡᴏʀᴋ Cʜᴀɴɴᴇʟs. Pʟᴇᴀsᴇ Jᴏɪɴ Tʜᴇ Cʜᴀɴɴᴇʟs Tᴏ Usᴇ Tʜᴇ Bᴏᴛ﹕\n\n@GenAnimeOfc, @FusionAnime, @DS_AnimeX,",
                reply_markup=InlineKeyboardMarkup(
                    [[
                        InlineKeyboardButton("Jᴏɪɴ", url=f"t.me/{sub_channel_id}")
                    ]]
                ),
            )
            return
        ...
        reply_markup = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton('🍁 ʀᴀɴᴅᴏᴍ', url='https://t.me/fusionanime')
                ]
            ]
        )
